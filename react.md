...
- SPA을 전제로 하고 있으며,
- Virtual DOM을 활용하여 업데이트 해야하는 DOM 요소를 찾아서 해당 부분만 업데이트하기 때문에, 리렌더링이 잦은 동적인 모던 웹에서 엄청나게 빠른 퍼포먼스를 내는게 가능합니다.
- 기본적으로 모듈형 개발이기 때문에 생산성 또한 상당히 높은 라이브러리인지라 순식간에 대세로 떠올랐습니다.
- 거기에 기본적으로 프레임워크가 아니라 라이브러리인지라 다른 프레임워크에 간편하게 붙여서 사용하는 것도 가능하며,
- React Hooks 라는 강력한 메소드들을 지원하면서 사실상 웹 프론트엔드 개발의 표준으로 자리잡았습니다.
- 이와 더불어 타입스크립트나 Sass 같은 언어도 지원합니다.
- 또한 Next.js등의 등장으로 인해 SSG, SSR등을 할 수 있게 되면서 사용 범위 또한 기하급수적으로 늘어났습니다.
...
- 리액트 방식으로 네이티브 앱을 개발할 수 있는 페이스북의 오픈소스 모바일 애플리케이션 프레임워크 입니다.
- 자바스크립트로 개발하여 iOS 나 안드로이드, 웹, UWP 에 대한 모바일 앱을 동시에 개발할 수 있는 크로스 플랫폼 입니다.

### 리액트 ↔️ 리액트 네이티브

- 리액트는 DOM 이 생성되고 난 뒤에 Virtual DOM 을 이용하여 변화된 곳을 캐치하여 변화된 DOM 으로 변경합니다.
- 실제 DOM 에 적용되기 전에 가상의 DOM 에 먼저 적용시키고 그 최종적인 결과를 실제 DOM 으로 전달해줍니다.
- 이로써, 브라우저 내에서 발생하는 연산의 양을 줄이면서 성능이 개선되는 것입니다.

<br>

- 리액트 네이티브는 코드를 실행하게 되면 JS Bridge 를 이용하여 iOS, 안드로이드 각각의 네이티브 언어에 접근할 수 있게 합니다.

1. 리액트와 리액트 네이티브는 둘 다 JSX 문법을 사용하지만, 리액트는 HTML 문법을 사용하고 리액트 네이티브는 HTML 문법을 사용하지 않습니다.

- 리액트 네이티브는 `<View>` `<Text>` `<TouchableOpacity>` 등 자체 태그를 사용합니다.

2. 리액트 네이티브는 기존의 CSS 를 지원하지 않고 `CreateStyleSheet` 를 이용한 스타일을 지원합니다.

- Styled-Component 등을 지원하기 때문에 무리없이 UI 구성이 가능합니다.
- flexbox 를 이용한 레이아웃 제작을 제공합니다.

### 리액트 네이티브 ↔️ Flutter

- Flutter 는 리액트 네이티브와 비슷한 성격을 가지고 있으나, Javascript 가 아닌 구글의 Dart 언어를 사용하기 때문에 러닝 커브가 높습니다.
- 하지만 훌륭한 공식 라이브러리 생태계를 가지고 있고, 쉽게 찾아볼 수 있게 잘 정리 되어 있기 때문에 손쉬운 개발이 가능합니다.
- 리액트 네이티브는 런타임 시 JS Bridge 를 통해서 컴파일 하는데, Flutter 는 Bridge 없이 Natvie 코드로 미리 컴파일 됩니다.
- 이런 컴파일적인 특징 덕분에 다른 크로스 플랫폼 보다 속도가 빠르다는 장점이 있습니다.

<br>

- 리액트 네이티브의 앱 스토어 심사없이 UI와 function 을 업데이트 할 수 있는 Code Push 기능은 하이브리드와 크로스 플랫폼 전체를 통틀어 리액트 네이트가 유일하기 때문에 큰 장점입니다.

### Expo ↔️ React Native CLI

#### Expo

- Expo-cli 은 보편적인 React 애플리케이션을 위한 프레임워크 이자 플랫폼 입니다.
- 동일한 JS/TypeJS 코드 베이스에서 iOS, 안드로이드 및 웹 앱을 개발, 빌드, 배포 및 빠르게 반복하는데 도움이 되는 리액트 네이티브 및 기본 플랫폼을 기반으로 구축된 도구 및 서비스 입니다.
- 장점
  - 초기 구성이 쉬워서 빠르고 간편하게 설치가 가능합니다.
  - 기본 설정이 미리 구성이 되어있습니다.
  - QR 코드를 통해서 해당 프로젝트를 실행할 수 있도록 도와줍니다.
  - 앱 스토어에 배포와 업데이트가 간편합니다.

<br>

- 단점

  - OS Layer 와 상호작용이 불가능 합니다.
  - Java, Kotlin, Object-C, Swift 로 작성된 네이티브 모듈을 추가할 수 없습니다.
  - APP 의 기본 파일 크기가 큽니다.
  - 기능이 많은 앱 개발에는 부적합 합니다.
  - 블루투스 이용이 불가합니다.
  - Eject 기능을 통해 설정하는 작업이 필요하여 오히려 번거로울 수 있습니다.

<br>

#### React Native CLI

- Xcode 또는 Android Studio 설치 및 설정이 필요합니다.

- 장점

  - 직접 네이티브로 어플리케이션을 개발할 수 있습니다.
  - 자기가 원하는 언어로 추가 개발이 가능합니다. - 커스텀 네이티브 모듈
  - OS Layer(Java, Kotlin, Object-C, Swift) 와 상호작용이 가능합니다.
  - 네이티브 파일들을 직접 다룰 수 있습니다.
  - 네이티브 모듈을 사용할 수 있습니다.
  - 필요한 기능이 있는 경우, 모듈을 직접 만들어 사용이 가능합니다.
  - Expo 패키지 이용이 가능합니다.
  - Android Studio 와 Xcode 에서도 빌드, 배포가 가능합니다.

- 단점

  - 초기 환경 구성이 오래걸립니다.
  - 프로젝트를 실행하려면 Android Studio 와 Xcode 모두 설치해야 빌드 및 배포가 가능합니다.
  - Mac OS 없으면 iOS 용 앱을 빌드할 수 없습니다.
  - 배포와 업데이트 시간이 상대적으로 오래 걸립니다.
  - 안드로이드, iOS 에 대한 폴더 구조에 대한 기본지식이 있어야 합니다.

<br>

- 리액트 네이티브 개발할 때 상황에 따라 적절한 CLI 를 선택해서 개발합니다.

### 리액트 네이티브에서 axios 말고 fetch 쓰는 이유

- axios 는 라이브러리로 다운받아 따로 import 해야 하지만, fetch 는 자바스크립트가 가진 내장 라이브러리 라서 따로 import 하지 않고도 쓸 수 있어서 빌드 시에 용량이 줄어듭니다.
- 또한 리액트 라이브러리는 버전이 빠르게 업데이트 되는 특성이 있기 때문에 최대한 라이브러리 의존성을 낮춰야 합니다.

### iOS, Android와 RN이 상호작용 하는 방식

- 리액트 네이티브에서 제공하는 Bridge 로 상호작용 하게 됩니다.
...
- 스택 네비게이션
  ...
  - 스택 네비게이션은 자료구조의 스택처럼 하나씩 쌓아가는 개념입니다.
  - 어떤 다른 페이지로 이동 한다고 했을 때 그 페이지로 완전히 전환되는 것이 아닌 그 페이지가 위에 쌓이는 것입니다.
  - 또 다른 페이지로 이동이 되면 쌓이고 뒤로가기 버튼을 통해 쌓인 스택을 하나 벗겨낼 수 있습니다.
  - 컴포넌트에 페이지 기능을 부여해주고, 컴포넌트에서 컴포넌트로, 즉 페이지 이동이 가능하게 해줍니다.
  - 컴포넌트를 페이지 처럼 사용할 수 있게 페이지로 컴포넌트를 감싸고,
  - 이렇게 만든 여러 페이지들을 스택 네이게이터에 모조리 등록시켜서 언제든 이동이 가능하게끔 해줍니다.
